"use client";

import { GlassCard } from "@/components/ui/glass-card";
import { cn } from "@/lib/utils";

interface MobileDashboardWidgetProps {
  label: string;
  value: string | number;
  icon: React.ElementType;
  trend?: string;
  color?: string;
  className?: string;
}

export function MobileDashboardWidget({
  label,
  value,
  icon: Icon,
  trend,
  color = "text-white",
  className,
}: MobileDashboardWidgetProps) {
  return (
    <GlassCard className={cn("p-5", className)}>
      <div className="flex justify-between items-start mb-3">
        <div className="p-2.5 rounded-sm bg-white/5 border border-white/5">
          <Icon className={cn("w-5 h-5 text-white")} />
        </div>
        {trend && (
          <span className="text-[10px] font-bold text-white bg-white/10 px-2 py-0.5 rounded-sm uppercase tracking-tighter">
            {trend}
          </span>
        )}
      </div>
      <div>
        <div className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] mb-1">{label}</div>
        <div className={cn("text-xl font-bold tracking-tighter text-white")}>{value}</div>
      </div>

      {/* Decorative background element */}
      <div className={cn(
        "absolute -bottom-2 -right-2 w-16 h-16 blur-3xl opacity-5 rounded-full bg-white"
      )} />
    </GlassCard>
  );
}
