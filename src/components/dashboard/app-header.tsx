    ...
    <header className="sticky top-0 z-40 w-full px-4 pt-4 pb-2 lg:px-8 lg:pt-6">
      <div className="max-w-7xl mx-auto flex items-center justify-between glass-morphism rounded-sm p-2 pr-3 pl-4 border border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.2)]">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {/* Credit Display */}
          <Link href="/dashboard/credits" className="flex items-center gap-1.5 px-2 py-1 sm:px-3 sm:py-1.5 rounded-sm bg-white/10 border border-white/20 text-white hover:bg-white/20 transition-all">
            <Wallet className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
            <span className="text-[10px] sm:text-xs font-bold font-mono">{(Number(profile?.paid_credits || 0) + Number(profile?.credit_balance || 0)).toFixed(2)} CR</span>
          </Link>
          ...
          >
            ...
            <span className="absolute top-2 right-2 w-2 h-2 bg-white rounded-full border-2 border-void shadow-[0_0_8px_#FFFFFF]" />
          </motion.button>
          ...
          <Link href="/dashboard/settings" className="flex items-center gap-2 sm:gap-3 ml-1 sm:ml-2 group">
            ...
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-sm bg-white/10 p-[1px]">
